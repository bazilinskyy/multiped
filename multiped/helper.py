...
class HMD_helper:
    ...
    def move_csv_files(self, participant_no, mapping):
        ...
        if not os.path.exists(participant_folder):
            os.makedirs(participant_folder)  # Use makedirs to ensure all directories are created
        ...
        participant_file_pattern = os.path.join(data_folder_path, f"{participant_no}*.csv")
        participant_files = glob.glob(participant_file_pattern)

        if participant_files:
            for participant_file in participant_files:
                dest_file = os.path.join(participant_folder, os.path.basename(participant_file))
    ...
    def plot_mean_trigger_value_right(readings_folder, mapping):
        participants_folders = [f for f in os.listdir(readings_folder) if os.path.isdir(os.path.join(readings_folder, f))]
        ...
        csv_files = [f for f in os.listdir(sample_folder) if f.startswith('video_') and f.endswith('.csv')]
